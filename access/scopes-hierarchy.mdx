...
```ts
import { GLOBAL_SCOPE } from "@alfiz-auth/core";
// GLOBAL_SCOPE === "*"
```
...
Alfiz cannot know your resource hierarchy — it lives in your tables. You expose it to the check engine through an `AncestryResolver`:
...
A grant anywhere in that list covers the target. The cost is O(depth) — one resolver call per ancestor level.

<Tip>
Scope types with `parent: null` in the catalog are flat by declaration. Their object closure is always `[scope, "*"]` — Alfiz can check them synchronously without consulting the resolver at all. Use `parent: null` for scope types whose instances don't nest (individual projects, top-level resources).
</Tip>

## The listing problem

Point checks (`can()`) answer "can Alice read this document?" — one check, one object closure walk. Listing pages answer a different question: "which documents can Alice read?" Naive per-row checking is an N+1 death: you cannot load every document and call `can()` on each one.
...
```ts
import { planListing } from "@alfiz-auth/core";
...
```ts
import { matPathCondition } from "@alfiz-auth/core";
...
```ts
import { prismaMatPathWhere } from "@alfiz-auth/core";
...
```ts
import { closureTableCondition } from "@alfiz-auth/core";
...
Without this call, the cache TTL (60 seconds by default) bounds how long a stale ancestor chain can persist. For sensitive operations — moving a confidential document into a restricted folder — the old chain surviving for up to a minute is unacceptable.
...
<Note>
Grants at **descendant** scopes are separate rows — Alfiz does not enumerate your subtree. When you delete a folder and all its contents, call `deleteScope` once per deleted resource id.
</Note>
