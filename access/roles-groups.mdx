...
const teachers = await app.createGroup(
  { name: "Teachers", description: "All teaching staff" },
  { kind: "admin", actorUserId: "root" },
);
...
// List members
const members = await app.getGroupMembers("group_teachers");
```

Group membership is stored on the user record as `groupIds: string[]`. At subject-closure computation time, every group in that list — and every ancestor of those groups — joins the closure.

### Group nesting (the inheritance DAG)

A group may declare parent groups. It inherits the union of all its parents' access. The parent graph must be a DAG — Alfiz enforces this transactionally at every `setGroupParents` call. Attempting to create a cycle throws a `GraphCycleError` with the full cycle path named:

```ts
await app.setGroupParents(
  "group_biology_teachers",
  ["group_teachers"],
  { kind: "admin", actorUserId: "root" },
);

// Now group_biology_teachers inherits everything group_teachers can do.
// If group_teachers has its own parents, those propagate too.
```
...
await app.setGroupParents(
  "group_biology_teachers",
  ["group_teachers", sharedAccess.id],
  { kind: "admin", actorUserId: "root" },
);
await app.setGroupParents(
  "group_guest_editors",
...
<Warning>
`canAny` is a **visibility affordance only** — never use it as an authorization gate. Every page and action must still call `alfiz.can(principal, key, scope)` or `alfiz.requirePermission(...)` with a concrete key. The static verifier (`alfiz-verify`) will error on `canAny` used in a gate position.
</Warning>
