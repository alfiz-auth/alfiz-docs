...
groupSubject("teachers")    // "group:teachers"
...
A concrete example: if Alice belongs to `group:teachers`, and `group:teachers` has parent `group:staff`, Alice's subject closure is:
...
Every member of `group:teachers` now holds `docs.files.update_file` at that scope through their subject closure. Group membership is stored on the user record as `groupIds: string[]`.
...
await app.setGroupParents(
  ...
  ["group_teachers"],   // inherits everything teachers has
  { kind: "admin", actorUserId: "root" },
);
```
...
// ["user:alice", "group:teachers", "group:staff", "everyone"]
```
