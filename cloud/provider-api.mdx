...
<Note>
  Before 0.6.0 this connection was the **relay module** — `createRelayHandler` and a single RPC endpoint. The seam is the same; what changed is that its wire form is now a normative, versioned API. If you are upgrading, see the 0.6.0 entry in the [changelog](/changelog) — the rename is mechanical, but both ends of a link must upgrade together.
</Note>
...
The whole API follows from the contract being method-shaped, so the API is too — one path per operation, mirroring the contract one-to-one, rather than a resource grammar bolted over an RPC contract:
...
The one deliberate exception to "the API is administrative traffic": a service that cannot run the TypeScript client — Go, Python, Java, anything — may POST a check and have the **serving Application** evaluate it in-process, against the same catalog, rows, resolver, and closure caches every local check uses.
...
Semantics are exactly `can`: an array key is any-of, no scope means the global scope, `fresh: true` bypasses the serving side's caches. Unknown keys answer with the typed-error envelope (`UnknownPermissionError` — a programming error, map it to 500), and keys declared [`requiresCondition: true`](/enforcement/conditions) answer with `MissingConditionError` rather than a half-checked yes — conditions are in-process predicates over resource state and cannot cross a wire.

What this does **not** change: "runtime checks never leave the application" survives intact, because the serving side *is* the Application, in your infrastructure — nothing Alfiz operates is on the path, and nothing here is metered. The caller pays a network hop instead of an in-process call, which is the honest cost of not being in the process. Keep latency-sensitive polyglot callers close to the Application, and let TypeScript services keep using the in-process client.
...
<Steps>
  ...
  <Step title="Configure the connection URL and copy the secret">
    Point the entry at the **base URL** your handler is mounted below — operations POST to `{base}/v1/{op}` — and link it. The provider secret is shown **once, at link time** — copy it into your deployment's environment as `ALFIZ_PROVIDER_SECRET`. Losing it means re-linking, not recovering.
  </Step>
...
Both halves of the wire ship in `@alfiz/application`: the handler that serves it, and the hosted provider that consumes it. `createHostedProvider` gives you an `AlfizProvider` over `fetch` — useful when an internal admin tool or a back-office service needs to administer an Application it has no database connection to:
...
Typed errors re-throw on this side exactly as the serving side threw them, and the far side's epoch is exposed, so caches revalidate across the wire the same way they do across processes. Transport failures — unreachable host, bad credentials, unknown op — surface as `ProviderTransportError`, distinct from errors the provider itself threw.
