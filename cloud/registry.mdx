...
→ "Role 'Teacher' references 3 permissions no longer published: zoom.breakout.manage, ..."
```
...
npx alfiz-verify codegen zoom.catalog.json
```

The generated types make cross-namespace references — a role bundling `mathaniyy.students.*` with `zoom.host` — compile-checked in the consuming codebase, exactly as local keys are.
