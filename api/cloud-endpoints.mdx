...
| PUT | `/orgs/{org}/org/groups/{id}/parents` | `alfiz_internal.access.manage_groups` | Set group parentage — a cycle returns `GraphCycleError` with the full path. |
| GET | `/orgs/{org}/org/groups/{id}/members` | `alfiz_internal.access.read` | Resolve group membership. |
...
| PUT | `/orgs/{org}/org/users/{id}/active` | `alfiz_internal.access.manage_reporting` | Activate or deactivate a user. |
...
| DELETE | `/orgs/{org}/org/scopes/{scope}` | `alfiz_internal.access.manage_grants` | Remove all rows for a deleted scope. |
