---
title: "defineCatalog() — Declare Your Permission Tree"
sidebarTitle: "defineCatalog()"
description: "defineCatalog(input) creates the Alfiz catalog object for your application — the source of truth for permission keys, scope types, navigation, and derived types."
---

`defineCatalog` is where your authorization model lives. It takes a structured declaration of your permission tree and produces a `Catalog` object that the Application validates writes against, the client checks keys against, and the build-time verifier consumes. The catalog is declared in code — not configured in a dashboard and not inferred from call sites — so it is always the ground truth.

`defineCatalog` throws `CatalogError` immediately on structural invalidity (bad segments, undeclared namespaces, missing scope types). Semantic convention violations — the naming floor, nav wiring — are reported by `alfiz-verify` at build time, not at runtime.
...
<ParamField body="namespace" type="string" required>
  Your application's primary namespace — the first segment of every permission key your application declares. Must be a single valid segment (`[a-zA-Z][a-zA-Z0-9_]*`). `alfiz_internal` is reserved.

  Example: `"docs"` produces keys like `docs.files.read`.
</ParamField>

<ParamField body="additionalNamespaces" type="string[]" optional>
  Additional namespaces this application owns. In a multi-project portal where you declare more than one top-level project, every project name must appear in either `namespace` or `additionalNamespaces`. Catalogs are federation-shaped from the first commit — declaring namespaces explicitly means the shape is portable to a federated deployment without refactoring.
</ParamField>

<ParamField body="projects" type="Record<string, GroupInput>" required>
  Top-level permission groups, one per project (namespace). The blessed structure is `project → tab → permission` — three levels. Each project key must match a declared namespace.

  See [Catalog Types](/api/catalog-types) for the full `GroupInput` field reference.
</ParamField>

<ParamField body="scopeTypes" type="Record<string, ScopeTypeInput>" optional>
  Scope types this catalog uses. Required for any catalog that grants permissions at resource scopes (anything other than global `"*"`). Keys are dotted identifiers like `"docs.folder"` or `"docs.doc"`.
...
<ParamField body="allowArbitraryDepth" type="boolean" default="false" optional>
  The blessed convention is exactly three levels (`project.tab.permission`). Set this to `true` to permit two-level integration catalogs (`zoom.host`) or deeper nesting. This is an explicit opt-out, not a default, so the linter does not treat it as an error on opted-in catalogs.
</ParamField>

<ParamField body="includeAlfizInternal" type="boolean" default="true" optional>
  When `true` (the default), Alfiz's own administration permissions are included under `alfiz_internal.*`. Set to `false` only for catalogs that render no Alfiz admin UI surface. When `false`, view-as and all other `alfiz_internal.*` features are unavailable — `createSession` will deny any view-as attempt because the permission key does not exist.
</ParamField>
...
<ResponseField name="Catalog" type="Catalog<C>">
  ...
  - `catalog.leaves` — `ReadonlyMap<string, LeafMeta>` of all permission keys
  - `catalog.groups` — `ReadonlyMap<string, GroupMeta>` of all group paths
  - `catalog.scopeTypes` — `ReadonlyMap<string, ScopeTypeMeta>`
...
function requirePermission(key: AppKey, scope?: string): Promise<void> { /* ... */ }
...
<Note>
  `alfiz_internal.*` keys are exempt from coverage linting in `alfiz-verify` — they are gated inside Alfiz's own admin surfaces, not your application code, so unreferenced warnings are suppressed for them automatically.
</Note>
...
export const catalog = defineCatalog({
  ...
  projects: {
    docs: {
      label: "Documents",
      description: "Document and folder access",
      groups: {
        files: {
          label: "Files",
