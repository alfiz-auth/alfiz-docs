...
`defineCatalog` derives its unions from the catalog *literal*. Code that consumes the published `CatalogDocument` instead — a federated sibling, another repo — gets the same treatment through codegen:
