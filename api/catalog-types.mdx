...
<ParamField body="namespace" type="string" required>
  Your application's primary namespace. Must be a single valid segment matching `/^[a-zA-Z][a-zA-Z0-9_]*$/`. Every top-level project key must match a declared namespace. `alfiz_internal` is reserved.
</ParamField>

<ParamField body="additionalNamespaces" type="readonly string[]" optional>
  Additional namespaces this application owns. Each project key that does not match `namespace` must appear here. Catalogs are federation-shaped from the first commit: declaring namespaces explicitly means the shape is portable to a federated deployment without a later refactor.
</ParamField>

<ParamField body="projects" type="Record<string, GroupInput>" required>
  Top-level permission groups, one per project. The blessed convention is `project → tab → permission` (three levels). Each project key must be a declared namespace.
</ParamField>
...
<ParamField body="allowArbitraryDepth" type="boolean" default="false" optional>
  By default, `defineCatalog` requires exactly three levels (`project.tab.permission`). Set to `true` to permit two-level integration catalogs (`zoom.host`) or deeper nesting. This is an explicit opt-out; the linter does not flag it on opted-in catalogs.
</ParamField>
...
A permission leaf is a concrete, grantable and checkable permission. Assign it to a key in `GroupInput.permissions`. The shorthand `true` is equivalent to an empty `PermissionLeafInput {}` and lets `defineCatalog` infer all fields.
...
<ParamField body="scopes" type="readonly ScopeType[]" optional>
  Scope types at which this permission is grantable, **in addition to the global scope `"*"`**. When omitted, inherits from the nearest enclosing `GroupInput` that declares `scopes`; falls back to `[]` (global-only) when no ancestor group declares scopes. Declare explicitly (including `[]` for global-only) to override the inherited default. Granting at an undeclared scope type is a validation error at write time.
</ParamField>
...
Groups are folders in the permission tree. They never carry keys themselves — only their leaves do. The blessed shape is `project → tab → permission`; the `groups` field inside `GroupInput` allows further nesting.
...
<ParamField body="permissions" type="Record<string, LeafInput>" optional>
  The leaves directly under this group. Keys are single segments (e.g., `"read"`, `"update_file"`) and become the final segment of the full permission key.
</ParamField>

<ParamField body="groups" type="Record<string, GroupInput>" optional>
  Nested child groups. Supports recursive nesting. When `allowArbitraryDepth` is `false` (the default), `defineCatalog` errors if any resulting key is not exactly three segments deep.
</ParamField>
