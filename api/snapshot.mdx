...
<ResponseField name="Promise<AlfizSnapshot<K, P>>" type="Promise<AlfizSnapshot>">
  ...
  <Expandable title="AlfizSnapshot members">
    ...
    <ResponseField name="canAny(pattern)" type="(pattern: P) => boolean">
      ...
      granted scope's chain was resolved at snapshot time.
    </ResponseField>
    ...
    <ResponseField name="holds(key)" type="(key: K) => boolean">
    ...
    <ResponseField name="grantedScopes(key)" type="(key: K) => { granted: Set<ScopeId>, revoked: Set<ScopeId> }">
      Synchronous listing primitive — same result as `client.grantedScopes`
      but without an async call.
    </ResponseField>
    <ResponseField name="explain(key, scope?)" type="(key: K, scope?) => CheckExplanation & { objectClosure, active, implied }">
...
```typescript
import type { SnapshotOf } from "@alfiz-auth/core";
