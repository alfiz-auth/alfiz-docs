...
<ParamField body="metrics" type="{ bucketMs?: number; retentionMs?: number }" optional>
  ...
  Requires a storage driver implementing `recordMetrics`, `readMetrics`, and `pruneMetrics` — the `AlfizMetric` model in the Prisma fragment. Construction throws when they are missing, on the same reasoning as `events.persist`: silently accepting metrics that go nowhere would make every safeguard read a confident zero.
