...
<ParamField body="partition" type="string" optional>
  Prefixes every collection this driver touches (`docs_grants`, `docs_epoch`, `docs_locks`, …) so several Applications can [share one `Db`](/operations/shared-store). A separate `Db` per application (`client.db("alfiz_docs")`) needs no option at all and is strictly preferred where databases are not rationed. Must match `[A-Za-z0-9_-]+`.
</ParamField>
