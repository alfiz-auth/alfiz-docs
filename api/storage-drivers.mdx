...
Two conventions a custom driver has to get right, because nothing type-checks them:

- **Absent rows return `null`** — not `undefined`, and never a thrown error. Both bundled drivers follow this.
- **`listAudit({ limit })` returns the NEWEST `limit` events**, not the oldest. The obvious `SELECT … ORDER BY at ASC LIMIT n` returns the wrong end of the log and silently makes every audit view show the beginning of time. The memory driver slices from the end (`rows.slice(-limit)`); the Prisma driver orders ascending and takes `-limit`.
