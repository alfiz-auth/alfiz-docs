...
<CardGroup cols={2}>
  <Card title="Cross-namespace role composition" icon="arrows-merge">
    Define a role like "Project Lead" that grants both `docs.files.*` and `zoom.host` in a single assignment. Cross-namespace composition requires the central catalog registry; it is not possible with independent applications. Each application declares the foreign slice it interfaces with as an [import](/catalog/imports), so those keys are typed and verified on its side too.
  </Card>
