...
When you federate, your Application acquires the Service as an **upstream**. That relationship has a precise shape:

- Your Application **publishes its catalog** to the Alfiz Cloud registry, where it becomes a versioned, namespaced contract.
- Your Application **syncs the org-root read model down** — groups, roles, global grants, the reporting hierarchy — and evaluates it locally exactly as it did when standalone.
- Your Application **keeps instance-scoped grants, ancestry resolution, and runtime checks strictly local**. The Service never receives resource-shaped data, never evaluates scoped checks, and never needs to.
...
## The no-dual-write property

Federation does not create two writers. The Service becomes the org root and the authoritative home for organizational-domain data. Each Application holds that data as a **synced read model** and rejects local org-domain writes. There is no reconciliation problem, no conflict resolution, and no clock skew between writers, because there is only one writer for any given piece of data.

For instance-scoped grants — `docs.doc:123`, `billing.invoice:456` — the opposite is true: they live in the owning Application and never replicate to the Service. The Service cannot evaluate scoped checks because it cannot see the application's resource tables. That is a design invariant, not a limitation: it is what keeps enforcement local and the check path unmeterable.

## What federation unlocks

<CardGroup cols={2}>
  <Card title="Cross-namespace role composition" icon="arrows-merge">
    Define a role like "Teacher" that grants both `mathaniyy.students.*` and `zoom.host` in a single assignment. Cross-namespace composition requires the central catalog registry; it is not possible with independent applications.
  </Card>
  <Card title="Catalog registry" icon="book-open">
...
If your Application is already linked to Alfiz Cloud, promotion to federation is the typical path — the relay machinery is already in place, and promotion adds only the authority transfer.

During promotion, your Application's organizational-domain data — groups, roles, global grants, the reporting hierarchy — imports into the Service through the same validation paths that already exist for external input. Authority transfers **atomically**: after the transfer completes, your Application receives the dataset back as a synced read model and rejects local org-domain writes.
...
Nothing on any application's request path is ever counted, priced, or throttled. That guarantee holds across the entire federation:

- Runtime checks run in-process in each Application against local data.
- The Service cannot observe checks, by construction.
...
<CardGroup cols={2}>
  ...
  <Card title="Link first" icon="link" href="/growth/linked">
    Linking is the typical first step — it adds the hosted dashboard and the relay, which promotion to federation builds on.
  </Card>
</CardGroup>
