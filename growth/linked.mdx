...
<Steps>
  ...
  <Step title="Configure the Alfiz Cloud connection in your Application">
    The connection is the **relay** — a POST route that ships in `@alfiz/application`. Mount it, and Alfiz Cloud forwards administrative operations through it to your Application for enforcement. Your `createApplication` options do not change.
...
The hosted dashboard is convenience, not capability. Every workflow it exposes is buildable from the headless admin kit included in `@alfiz/core` — the hosted product is assembled from the same components, speaking the same provider contract.
