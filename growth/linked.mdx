...
<CardGroup cols={2}>
  <Card title="Diwan hosted dashboard" icon="layout-dashboard">
    A styled, maintained admin dashboard that non-engineer administrators can use to manage roles, groups, grants, and access requests — assembled from the same headless kit your own admin UI would use, backed by your Application.
  </Card>
...
Alfiz Cloud cannot observe your runtime checks in any topology. Per-check pricing is impossible by construction, and that is a guarantee, not a policy.
...
<Steps>
  ...
  <Step title="Register your Application">
    From the Diwan dashboard, create a linked application entry and copy your connection credentials. These connect your Application to Alfiz Cloud — they are not used for runtime checks.
  </Step>

  <Step title="Configure the Alfiz Cloud connection in your Application">
    Pass your connection credentials when constructing the Application. Everything else remains identical to your standalone setup.
...
<Card title="Federating applications" icon="share-nodes" href="/growth/federated">
  Federation moves the org root to Alfiz Cloud, unlocking cross-application role composition, a shared catalog registry, and provision-and-reconcile integrations. Promotion from the linked state is the typical path — the connection to Alfiz Cloud is already established.
</Card>
