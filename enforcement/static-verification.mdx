...
export async function assertTeaches(userId: string, courseId: string) {
  await alfiz.requirePermission({ userId }, "courses.content.edit", `courses.course:${courseId}`);
}
```

```json
{
  "gateNames": ["gateDestructiveAction", "assertTeaches"]
}
```
