...
Gate every page handler or server component at the top, before any data fetching. Use `requirePermission` with the concrete key for the page's primary action. For project root pages — where the right question is "does this user have _anything_ under this project?" — use `requireAny` to gate on the subtree pattern, but still call `requirePermission` for the page's own read:
...
export default async function DocumentPage({ params }: { params: { id: string } }) {
  ...
  await alfiz.requirePermission({ userId: session.userId }, "docs.files.read", scope);
...
const navItems = await Promise.all(
  catalog.navigation.map(async (item) => ({
    ...item,
    visible: await alfiz.canAny(principal, item.permission),
  }))
);
...
<Note>
`canAny` is the only valid shape for navigation visibility checks. It answers the broad "does this principal hold anything under this pattern?" question. Do not use `can` for nav visibility — it requires a concrete scope that the navigation layer does not have. Do not use `canAny` as a gate anywhere else.
</Note>
...
<Tabs>
  ...
  <Tab title="Route handler">
    ...
    import { alfiz } from "@/lib/alfiz";
    import { getSession } from "@/lib/session";
    import { isAccessDenied } from "@alfiz-auth/core";
...
const snap = await alfiz.snapshot({ userId: session.userId });
...
| Server action | `requirePermission(principal, key, scope)` | The resource being mutated |
...
<Note>
`alfiz-verify` checks coverage across all four points at build time. Run it in CI to catch ungated server actions, misused `canAny()` calls, and catalog keys that no gate references. See [Static Verification](/enforcement/static-verification) for configuration details.
</Note>
