...
For unscoped conditional UI, meaning buttons that should appear whenever the user _holds_ the permission at any scope, use `snap.holds(key)` rather than `snap.can(key)` without a scope. An editor who can update documents only in specific folders should still see the Edit button when it is relevant:
