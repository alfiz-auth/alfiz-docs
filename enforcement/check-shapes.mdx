...
**`key`** is a typed template-literal derived from your catalog. The TypeScript compiler rejects any string that is not a declared leaf. You can also pass a `readonly` array of keys, and `can` returns `true` when the principal holds _any one_ of them:
