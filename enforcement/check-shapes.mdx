---
title: "Check Shapes: can, canAny, and can.fresh"
...
`can` returns `Promise<boolean>`. It never throws on a denied check; use `requirePermission` when you want a throw instead.
...
<Warning>
**`canAny` is never a gate.** It tells you whether a section is worth showing — it does not authorize any action. The `alfiz-verify` static verifier emits an error whenever `canAny` appears inside a server action or route handler. Gate on a concrete key with `can` or `requirePermission`.
</Warning>

---

## `requirePermission` and `requireAny`

`requirePermission` and `requireAny` are the throwing forms of `can` and `canAny`. They return `Promise<void>` and throw `AccessDeniedError` when the check fails.

<CodeGroup>
```ts requirePermission
...
```ts
import { AccessDeniedError } from "@alfiz-auth/core";

try {
  await alfiz.requirePermission({ userId }, "docs.files.delete", "docs.folder:9");
} catch (err) {
...
Use `requirePermission` at the top of server actions and route handlers where you want a single thrown value to carry all downstream error handling. Use the boolean `can` when you need to branch without catching.

---

## `snapshot(principal)` — one round-trip, synchronous checks
...
|---|---|---|
| Subject closures | 30 s | Provider `user`, `subject`, `role`, or `all` invalidation event |
