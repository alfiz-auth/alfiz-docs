...
Log order is **(`at`, then `id`)** — two entries in the same millisecond order by id, identically in every driver, which is what makes cursor paging stable.
...
<Warning>
Do not confuse this with the invalidation **event log** (`AlfizEvent`), which has documented retention defaults of 7 days / 100 000 rows. That log is cache-freshness plumbing and pruning it affects nothing but cache catch-up. Event-log pruning never touches `AlfizAudit` — they are different tables with different jobs.
</Warning>
...
With the chain on, every entry carries `hash` — SHA-256 over the entry's canonical serialization plus the previous entry's `hash` (`prevHash`). Editing, deleting, or reordering any entry breaks every hash after it:
...
- **It is evidence, not proof.** An attacker with database write access *and* this source code can rewrite and re-hash the whole chain. What makes the evidence hard is anchoring the head hash somewhere they cannot reach — a ticket, a WORM bucket, a signed release note. That anchor is deliberately yours.
