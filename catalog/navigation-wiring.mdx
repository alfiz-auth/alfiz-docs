...
This evaluation happens client-side — it is a **display optimization**, not a security gate. The page the nav item points to must gate itself independently with `require` on a concrete key. Visibility and access are separate concerns, enforced separately.
...
export default async function AdminPage() {
  await alfiz.require(actor, "admin.users.read");
  // ...
}
