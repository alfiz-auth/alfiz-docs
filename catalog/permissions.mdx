...
2. **Action permissions are named `<verb>_<noun>`** in `snake_case` — `invite_user`, `export_report`, `publish_file`. A short list of common standalone verbs (`create`, `update`, `delete`, `manage`, `approve`, `publish`, `archive`, `export`, `import`, `issue`, `revoke`, `view_as`) are allowed without a noun.
...
// ✅ Well-formed tab
permissions: {
  read:               { kind: "read", description: "View the file list" },
  read_collaborators: { kind: "read", description: "View who a file is shared with" },
  create:             { description: "Create a new file" },
  publish:            { description: "Publish a draft file" },
  archive:            { description: "Archive a file" },
  delete:             { destructive: true, description: "Permanently delete a file" },
}

// ❌ Below the naming floor — no read permission
permissions: {
  create: { description: "Create a file" },
  publish: { description: "Publish a file" },
}
```
