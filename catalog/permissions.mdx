...
description: "Learn Alfiz permission declarations: dotted keys, read vs. action kinds, destructive flags, scope wiring, and forward-inclusive wildcards."
---
...
- The **first** segment is the namespace — declared, and enforced.
...
Depth is a **house-style setting**, not a rule. `conventions.depth` defaults to `3`, and `lintCatalog` is the only thing that reads it — `defineCatalog` never checks it, and neither does any check at runtime. If your keys are shaped differently, say so in one line rather than renaming to match a default:

```ts
conventions: { depth: 2 }      // an integration catalog: zoom.host
conventions: { depth: 4 }      // a deeper feature tree
conventions: { depth: "any" }  // mixed depths — turn the check off
```
