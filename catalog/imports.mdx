...
The severity is an **error** when no import source is configured — an application that has never imported anything has no plausible source for a foreign key, so it is a typo — and a **warning** when one is, naming the declaration to paste. Configure it in `alfiz-verify.config.json`:
