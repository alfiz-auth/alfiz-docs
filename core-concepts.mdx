...
The **naming floor** is checked by `lintCatalog`, and failed in CI by `@alfiz/verify`. Two of its rules are errors that fail the build: every group must have at least one `read` permission, and every key must match the catalog's declared depth convention **exactly** (three levels by default). Two are warnings: read leaves named other than `read` / `read_<thing>`, and actions not named `<verb>_<noun>` in snake_case. Standing destructive actions alone is a convention the catalog *supports* — `destructive` is inferred for `delete` and `delete_*`, and the action-naming rule explicitly permits a bare destructive verb — but no lint rule enforces the split.
