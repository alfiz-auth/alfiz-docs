...
The **naming floor** is enforced at build time by `@alfiz-auth/verify`: every tab must have at least one `read` permission; actions follow `<verb>_<noun>` in snake_case; destructive actions (`delete`, `delete_*`) stand alone as their own leaf; and keys match the catalog's declared depth convention (three levels by default).
...
```ts
// Global check: does the user hold docs.files.read at any scope?
