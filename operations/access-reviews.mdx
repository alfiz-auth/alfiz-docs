...
What a reviewer reads: capability (`held`), not rows. What remediation uses: the `sources` — each names the grant row to revoke or end-date, and a personal revoke covers the "remove this one person from inherited access" case. **Write-back is the ordinary API**; there is no parallel remediation surface to integrate.
...
The review loop: record the catalog version each cycle certifies; open the next cycle with `listWildcardDrift({ sinceVersion })`; every finding is a grant or assigned role now conferring more than what was signed. Expired grants and unassigned roles are excluded — they confer nothing. `removedKeys` surfaces tombstone candidates in the other direction.
...
- **Remediate:** the IGA's revoke decision calls your endpoint, which calls `deleteGrant` / `createRevoke` / an expiry update — audited like every write. A review that cannot remediate is not a control; this one closes the loop with three ordinary calls.
...
First-party campaign objects — population snapshots with reviewer state machines — would duplicate the half of IGA tooling that is workflow, not authorization. The primitives above are the half that must come from the authorization layer because only it can compute them. If you operate no IGA, the export plus a spreadsheet and the audit trail is a defensible small-company review; the ingredients do not expire if you later adopt one.
