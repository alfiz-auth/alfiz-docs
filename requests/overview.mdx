...
## The headless admin surfaces

The headless admin kit exposes two ready-made request surfaces:

- **Request form** — renders the justification prompts declared on the requestable role or scope type, validates required fields and select options before submission, and calls `submitRequest` for you.
- **Approver queue** — shows each approver their pending decisions, with the proposed grant details and justification answers, and wires the approve/deny actions to `decideRequest`.

Both surfaces are headless: they own logic and state, not markup. Bring your own components.
