...
When a requestability policy is attached to a **namespace node** in the hierarchy, it governs every request under that node unless a nearer node declares its own policy. This applies to scope-type policies: a policy on a parent scope type does not propagate to child scope types — each scope type declares its own `requestable` block independently.
